...
@router.callback_query(F.data == "dry_uk")
async def send_uk_dry(callback: types.CallbackQuery):
    await callback.message.edit_text(text='От какого производителя интересен сидр?', reply_markup=uk_dry_cider_btn())


def uk_dry_cider_btn():
    buttons = [
        [types.InlineKeyboardButton(text="Henney's", callback_data="henney_dry")],
        [types.InlineKeyboardButton(text='ROSS-ON-WYE CIDER &PERRY', callback_data="RoW_dry")],
        [types.InlineKeyboardButton(text='Hecks Cider', callback_data="hecks_dry")],
...
async def send_manufactor_semidry(callback: types.CallbackQuery, manufacture: str, sour="Сухой"):
